...
export const App = (): JSX.Element => {
    ...
    return (
        <main className="flex-grow p-6 flex flex-col items-center min-h-screen bg-primary-50 text-gray-400 font-bold">
            <Card className="w-2xl h-[500px] bg-primary-700 text-white border-none rounded-xl shadow-2xl flex flex-col relative overflow-hidden">
                {/* Tabs menu */}
                    ...
                    {tabs.map((tab, idx) => (
                        <button
                            ...
                            className={`flex-1 py-3 text-lg font-semibold transition-colors
                                ${activeTab === idx ? 'border-b-4 border-blue-400 text-blue-200' : 'text-white hover:text-blue-300'}
                                bg-transparent`}
